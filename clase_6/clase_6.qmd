...
archivo_excel = 'C:/Users/Brad/Documents/curso_python_poli_dec/data/co2.xlsx'
...
Sirve para mostrar valores que surgen de la intersección de dos variables y es recomendable usarlo cuando queramos resaltar ciertos valores.

```{python}
#Cargamos una librería de bases de datos 
from sklearn import datasets

cigs=data('Cigarette')
#Creamos la matriz de correlación de forma primitiva
corr=cigs.corr()
#Vamos a usar matplotlib para crear esta matriz de una mejor forma 
#Creamos la matriz
fig, ax = plt.subplots(figsize=(8,8))
im = ax.imshow(corr, vmin=-1,vmax=1,cmap="Blues")

#Creamos una leyenda que ayude a la visualización
cbar = ax.figure.colorbar(im,ax=ax)
cbar.ax.set_ylabel("Leyenda",rotation=-90)

ax.set_xticks(np.arange(len(corr.columns)))
ax.set_yticks(np.arange(len(corr.index)))

#Ponemos las etiquetas a los ejes
...
#Ponemos el título y mostramos
ax.set_title("Gráfico de correlación")
fig.tight_layout()
plt.show()
```

Ahora veamos cómo con seaborn podemos ahorrar mucho trabajo

```{python}
# Usamos seaborn para hacer la matriz de correlación mucho más fácil 
