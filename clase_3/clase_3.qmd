...
```{python}
df_agrupado = df.groupby('A').agg({'C': 'sum'})
df_agrupado = df.groupby('B').agg({'C': 'sum'})
df_agrupado = df.groupby('D').agg({'C': 'sum'})
df_agrupado = df.groupby('E').agg({'C': 'sum'})
```

Las oepraciones son las mismas pero cambia la variable de grupo.

Para eso empleamos funciones:
...
Aquí tienes un ejemplo de una función bien escrita siguiendo estas prácticas:
